...
function ensureFirstSphereSetup(user_id, site_id, node_id, meta_data) {
  ...
  // only attempt to promote the current node
  var sql = 'UPDATE `sites` SET `master_node_id` = ? WHERE site_id = ? AND user_id = ? AND master_node_id IS NULL;';
  return this.service.facet('mysql').then(function(mysql) {
    return mysql.query(sql, [node_id, site_id, user_id]).then(function(){
      return site_id;
...
function listTokensFromID(user_id) {
  var idSql = 'SELECT * FROM `accesstoken` WHERE userId=? AND type=? ORDER by mqtt_client_id DESC LIMIT 1;';
  return this.service.facet('idMysql').then(function(idMysql){
    return idMysql.query(idSql, [user_id, 'node']);
  })
}
