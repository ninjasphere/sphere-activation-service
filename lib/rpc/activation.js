...
*/
function ensureFirstSphereSetup(user_id, site_id, node_id, potential_usurper) {
  if (!potential_usurper) {
    return site_id;
  }
...
function addClaimedNode(claim, metaData) {
  ...
  var nodeAdded = this.service.facet('mysql').then(function(mysql) {
    return mysql.query(nodeSQL, [claim.user_id, claim.site_id, claim.node_id, hardware_type]).then(
        ensureFirstSphereSetup.call(this, claim.user_id, claim.site_id, claim.node_id, metaData.potential_usurper)
    );
  }.bind(this));
...
function notifyPairingEvent(user_id, node_id, event) {
  ...
  return this.service.facet('amqp').publish('amq.topic', routingKey, {userId: user_id, node_id: node_id});
}
