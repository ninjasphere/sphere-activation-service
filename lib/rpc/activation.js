...
function ensureUser(user) {
  var sql = 'INSERT INTO `users` (user_id, name, email, lastAccessToken) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE name=?, email=?, lastAccessToken=?;';

  return this.service.facet('mysql').then(function(mysql) {
    return mysql.query(sql, [user.id, user.name, user.email, user.accessToken, user.name, user.email, user.accessToken]);
  });
}
