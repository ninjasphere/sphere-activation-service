...
function addClaimedNode(claim, metaData) {
  ...
  var nodeSQL = 'INSERT INTO `nodes` (user_id, site_id, node_id, hardware_type) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE site_id=?, hardware_type=?;';
  var nodeAdded = this.service.facet('mysql').then(function(mysql) {
    return mysql.query(nodeSQL, [claim.user_id, claim.site_id, claim.node_id, hardware_type, claim.site_id, hardware_type]).then(
