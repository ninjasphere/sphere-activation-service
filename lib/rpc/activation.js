...
function ensureFirstSphereSetup(user_id, site_id, node_id, meta_data) {
  ...
  var sql = 'UPDATE `sites` SET `master_node_id` = (SELECT `node_id` FROM `nodes` WHERE site_id = ? AND user_id = ? ORDER BY `updated` DESC LIMIT 1) WHERE site_id = ? AND user_id = ? AND master_node_id IS NULL;';
  return this.service.facet('mysql').then(function(mysql) {
    return mysql.query(sql, [site_id, user_id, site_id, user_id]).then(function(){
      return site_id;
    });
  });
}
...
function getConnectionStatus(result) {
  this.service.facet('redis').then(function(redis){
