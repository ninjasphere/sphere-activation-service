      ...
      return retryAtInterval(function(deferred) {
        return redis.client.get('node:'+nodeId+':'+user.id+':claimed').then(function(value) {
          ...
          return redis.client.del('node:'+nodeId+':'+user.id+':claimed').then(function() {
            deferred.resolve({user_id: user.id, site_id: site_id, node_id: nodeId, metadata: nodeMetadata});
            return true;
          });
        });
      }.bind(this), 500, 20000); // 500ms polling, 20s timeout
...
function ensureDefaultSite(user, firstNodeId) {
  return this.service.facet('mysql').then(function(mysql) {
    return mysql.query('SELECT site_id FROM `sites` WHERE user_id=?;', [user.id]).then(function(results) {
      if (results.length > 0) {
        ...
        var site_id = uuid.v4();
        return mysql.query('INSERT INTO `sites` (user_id, site_id, master_site_id) VALUES (?,?,?)', [user.id, site_id, firstNodeId]).then(function() {
          return site_id;
        });
      }
    });
...
function getSite(user_id, site_id) {
  ...
  return this.service.facet('mysql').then(function(mysql) {
    return mysql.query(sql, [user_id, node_id]).then(function(results) {
      if (results.length > 0) {
        return results[0];
      } else {
