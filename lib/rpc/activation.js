...
module.exports.listUserActivatedNodes = function(user) {
  return listNodes.call(this, user.id).then(function(results) {
    debug('listUserActivatedNodes', results);
    return results.map(function(result) {
      return getConnectionStatus.call(this, result).then(function(status){
        return {
          type: 'node',
...
module.exports.userDeactivateNode = function(user, nodeId) {
  ...
  return when.all([
    removeNode.call(this, user.id, nodeId),
    notifyPairingEvent.call(this, user.id, nodeId, 'unpair')
  ]).then(function() {
    return {success: true};
  });
};
...
/*
 * When a node is paired the newest one is assigned master if one is NOT set and it matches the hardware_type
 *
 * TODO: May need to change masterNodeHardwareType to an array and map over it to select a master.
*/
function ensureFirstSphereSetup(user_id, site_id, node_id, meta_data) {
  // check for the usurper flag and legacy 'custom-linux-arm' which is a spheramid..
  if (meta_data.potential_usurper && meta_data.hardware_type !== masterNodeHardwareType) {
    return site_id;
  }
  debug('ensureFirstSphereSetup', 'NOTE', node_id, 'potential_usurper');
  var sql = 'UPDATE `sites` SET `master_node_id` = (SELECT `node_id` FROM `nodes` WHERE site_id = ? AND user_id = ? and node_id = ? ORDER BY `updated` DESC LIMIT 1) WHERE site_id = ? AND user_id = ? AND master_node_id IS NULL;';
  return this.service.facet('mysql').then(function(mysql) {
    return mysql.query(sql, [site_id, user_id, node_id, site_id, user_id]).then(function(){
      return site_id;
    });
  });
}
...
function addClaimedNode(claim, metaData) {
  ...
  var nodeAdded = this.service.facet('mysql').then(function(mysql) {
    return mysql.query(nodeSQL, [claim.user_id, claim.site_id, claim.node_id, hardware_type]).then(
        ensureFirstSphereSetup.call(this, claim.user_id, claim.site_id, claim.node_id, metaData)
    );
  }.bind(this));
...
function listNodes(user_id) {
  return when.all([
      listNodesFromDB.call(this, user_id),
      listTokensFromID.call(this, user_id)
    ]).spread(function(nodes, tokens){
      return nodes.map(function(node) {
        var res = tokens.filter(function(token){
          debug('listNodes', 'token', token);
          return token.node_id === node.node_id
        });
        if (res.length == 1) {
          node.mqtt_client_id = res[0].mqtt_client_id;
        }
        return node;
      });
    }.bind(this));
...
function listNodesFromDB(user_id) {
...
function listTokensFromID(user_id) {
  var idSql = 'SELECT * FROM `accesstoken` WHERE userId=?;';
  return this.service.facet('idMysql').then(function(idMysql){
    return idMysql.query(idSql, [user_id])
  })
}


// retrieve the connection status record if present from redis
function getConnectionStatus(result) {
  return this.service.facet('redis').then(function(redis){
    // mqtt:sessions:{mqtt_client_id} where mqtt_client_id matches what is in the DB
    return redis.client.get('mqtt:sessions:'+result.mqtt_client_id).then(function(value) {
...
function removeNode(user_id, node_id) {
  debug('removeNode', user_id, node_id);
  ...
  return this.service.facet('mysql').then(function(mysql) {
    return when.all([
      ...
      mysql.query(masterSiteUpdate, [node_id, user_id]) // IF the user_id and node_id match then remove the master
    ])
  });
}

function notifyPairingEvent(user_id, node_id, event) {
  var routingKey = user_id + '.$cloud.' + node_id + '.' + event;

  debug('notify', user_id, 'node', node_id, 'of', event);
  debug('routingKey', routingKey);

  var payload = new Buffer(JSON.stringify({userId: user_id, node_id: node_id}));

  // get the amqp service
  // and publish a message to $cloud/:nodeId/:event
  return this.service.facet('amqp').then(function(amqp) {
    return amqp.publish('amq.topic', routingKey, payload);
  });
}
