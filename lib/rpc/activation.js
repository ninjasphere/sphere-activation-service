...
function ensureUser(user) {
  ...
  return when.all([
    ...
    mysql.query(sql, [user.id, user.name, user.email, user.accessToken, sphereToken,
      ...
      return mysql.query('SELECT sphere_network_key FROM `users` where user_id=?', [user.id]).then(function (results) {
        return results.sphere_network_key;
      });
    });
  });
}
...
function ensureDefaultSite(user, node_id) {
  return this.service.facet('mysql').then(function(mysql) {
    return mysql.query('SELECT site_id FROM `sites` WHERE user_id=?;', [user.id]).then(function(results) {
      if (results.length > 0) {
        return ensureFirstSphereSetup(user.id, results[0].site_id, node_id);
      } else {
        var site_id = uuid.v4();
        return mysql.query('INSERT INTO `sites` (user_id, site_id) VALUES (?,?)', [user.id, site_id]).then(
          ensureFirstSphereSetup(user.id, site_id, node_id)
        );
      }
    });
  });
...
function ensureFirstSphereSetup(user_id, site_id, node_id) {
  var sql = 'UPDATE `sites` SET `master_node_id` = (SELECT `node_id` FROM `nodes` WHERE hardware_type = ? ORDER BY `updated` ASC LIMIT 1) WHERE site_id = ? AND user_id = ? AND master_node_id IS NULL;';
  return this.service.facet('mysql').then(function(mysql) {
    return mysql.query(sql, [masterNodeHardwareType, site_id, user_id]).then(function(){
      return site_id;
    });
  });
}
...
function addClaimedNode(claim, metaData) {
  ...
  var nodeAdded = this.service.facet('mysql').then(function(mysql) {
    return mysql.query(nodeSQL, [claim.user_id, claim.site_id, claim.node_id, hardware_type]);
  });
...
function removeNode(user_id, node_id) {
  ...
  var masterSiteUpdate = 'UPDATE `sites` SET master_site_id = NULL where master_site_id = ? AND user_id = ?';
  return this.service.facet('mysql').then(function(mysql) {
    return when.all([
